...
pnpm generate-pwa-icons  # regenerate favicon/browser/PWA icons from assets/app-icon.png (deterministic via sharp)
...
- **语言偏好持久化：** next-intl 的 `NEXT_LOCALE` cookie（1 年 `maxAge`，配置在 `i18n/routing.ts` 的 `localeCookie`）记住用户语言选择。`/` 的检测优先级：URL locale 前缀 → cookie → `Accept-Language` → 默认 `zh`。语言切换器的软导航由 next-intl 自动同步 cookie（middleware 与客户端两条写入路径共用同一配置），无需手动种 cookie。
...
- **PWA:** no-op service worker at `public/sw.js` (registered by `components/pwa/ServiceWorkerRegister.tsx`); web manifest at `app/manifest.ts`; **all icons** are generated by `pnpm generate-pwa-icons` from the master source `assets/app-icon.png` (deterministic via `sharp`) - the browser files (`app/icon.png`, multi-frame `app/favicon.ico`, `app/apple-icon.png`) plus the `public/icon-*.png` PWA set. `themeColor` MUST live in the `viewport` export, NOT `generateMetadata` (Next.js 16 moved it - configuring it in metadata emits a build warning and the `<meta name="theme-color">` tags are not rendered).
...
- **`proxy.ts` matcher excludes every dotted path** - only `/feed.xml` is added back by an explicit literal entry. Locale detection therefore runs for locale-less HTML paths (`/posts/hello-world` → 307 → `/{detected}/posts/hello-world`; detection order: `NEXT_LOCALE` cookie → `Accept-Language` → `zh`) but NEVER for `/{locale}/posts/{slug}/index.md`, `/llms.txt`, or `/{locale}/feed.xml`. Consequence: an unprefixed dotted path has no route to fall back on and hits the **default** (non-localized) 404 - `/posts/{slug}/index.md` matches nothing (`app/[lang]/posts/[slug]/index.md/route.ts` is 4 segments). `buildSharePostPath` in `lib/seo.ts` is the only place that intentionally emits a locale-less path (and only when the post exists in every locale, so single-locale posts keep the sharer's prefix instead of 404-ing for other readers); do not extend that pattern to markdown URLs.
...
- **Never re-add `theme_color` to the PWA manifest (`app/manifest.ts`)** - Android installed-PWA chrome (status bar / splash) is styled from install-time manifest metadata, not from the running page: Chrome ignores page `<meta name="theme-color">` in dark mode, and a declared manifest color applies in BOTH color schemes (`dark_theme_color` is obsolete in Blink). Omitting it on purpose makes Chrome use the platform surface color (`colorSurface`: `#FFFFFF` / `#131314` baseline, overridden by Material You dynamic colors - `#FAF9FE`-like light on some devices, so the exact value is device/theme-dependent); `background_color` (`#03142E`) still paints the splash. The platform still derives the status bar **icon tint** from the page's resolved `theme-color` in light scheme mode, so `components/theme/ThemeColorSync.tsx` suppresses its runtime tag in the installed PWA (`display-mode: standalone`) and lets the static `prefers-color-scheme` pair govern there - the page color follows the system scheme, keeping the icons legible against the system-driven chrome. In browser tabs the runtime tag still follows the in-app theme.
