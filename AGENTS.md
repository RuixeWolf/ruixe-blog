...
- **Never re-add `theme_color` to the PWA manifest (`app/manifest.ts`)** - Android installed-PWA chrome (status bar / splash) is styled from install-time manifest metadata, not from the running page: Chrome ignores page `<meta name="theme-color">` in dark mode, and a declared manifest color applies in BOTH color schemes (`dark_theme_color` is obsolete in Blink). Omitting it on purpose makes Chrome use its scheme-correct defaults (`#FFFFFF` light / `#000000` dark); `background_color` (`#03142E`) still paints the splash. `components/theme/ThemeColorSync.tsx` only affects surfaces that honor page colors.
