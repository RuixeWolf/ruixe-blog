---
title: 我的个人博客网站的诞生
description: 从大学时的构想到正式上线，记录我的个人博客网站从需求规划、技术选型、三阶段开发到正式上线的完整历程，以及 AI Coding 工作流在其中的实践。
...
- 支持 RSS 订阅，可以使用第三方文章阅读器订阅网站博客内容的变更
- 支持 PWA，可将网站添加至操作系统桌面，实现快捷打开，不需要过多缓存数据
- 支持 llms.txt，提升网站内容 AI 访问的友好度
- 尽量降低运维部署成本
...
我计划使用 CDN 云服务，不把图片等媒体资源存放在 Git 仓库，且考虑全球范围的可访问性，以下是几个云服务厂商的方案：
...
在 Vercel 导入 Github 项目仓库一键部署，网站访问地址 https://ruixe-blog.vercel.app，正式上线前为 Vercel 部署添加了自定义域名 https://blog.ruixe.net 作为博客网站的正式访问域名，新老域名都可以访问。
...
二阶段是博客网站的增强功能，开发过程中总共创建了 4 个 OpenSpec Changes
...
- 发布文章流程：
  - 补全或完善文章 Markdown 文件的 Fontmatter
...
三阶段实现了博客网站锦上添花的功能，开发过程中总共创建了 4 个 OpenSpec Changes

- **2026-08-12-add-seo-optimization** - 实现 SEO 优化，包含 sitemap 等功能
- **2026-08-13-add-rss-feed** - 实现 RSS 功能
- **2026-08-14-add-llms-txt** - 实现 llms.txt 功能
- **2026-08-14-add-pwa** - 实现 PWA 功能

### 正式上线

**自定义域名配置**

在 Vercel 的控制台添加了博客网站自定义访问域名 `blog.ruixe.net`，并在 CloudFlare 配置 DNS 解析。在 CloudFlare 配置 Vercel 项目的 DNS 解析时，Vercel 推荐不要开启 CloudFlare 的代理服务（小黄云），使用 DNS only 模式，否则 Vercel 无法完成域名 SSL 证书签发，影响流量统计等功能。

**正式文章创建**

删除了开发阶段的测试文章 `hello-world`，使用 Agent Skill `publish-post` 让 AI 辅助将草稿文章创建为正式文章。

**SEO 站长配置**

实现在 `site.yaml` 配置不同搜索引擎的站长管理的网站归属验证 Token 自动渲染 HTML meta 标签，并在 Google Search Console 与 Microsoft Bing Webmaster 添加了我的博客网站。

## 总结与展望

历时一个月，从写这篇文章到博客网站正式上线，中途利用空闲时间断断续续推进项目开发。终于将一个我还是学生时的理想实现了，我现在有了一个属于自己的技术博客网站，不再受大平台的内容审查限制，也不需要担心自己的博客文章被大平台用于作者不知情的付费阅读营收。当然，不在大平台发布文章意味着我需要自己维护博客网站的项目，需要自己处理博客内容曝光率和用户流量，但这也能让我学到很多新的知识。

在这个博客网站规划与开发的过程中，我了解到了 Next.js 开发与部署 Markdown 文件驱动型网站的全流程，了解了新的技术栈和很多博客网站相关的知识，也积累了 Next.js 开发、Vercel 部署、CloudFlare 操作等实践经验，还开始培养自己记录知识库文章的习惯。

后续我会在我的博客网站继续发布新的文章，分享技术内容的同时也作为我自己的知识库。未来我也会持续完善这个博客网站，与时俱进。

### 感谢

在博客的功能与 UI 设计阶段参考了 [碳苯 Carbon](https://github.com/CoderSerio) 的博客网站 https://furrycoder.com 和 阮一峰 的博客网站 https://www.ruanyifeng.com/blog，当然还有我和 [RylinWolf](https://github.com/RylinWolf) 共同开发的 CMS 博客系统 Wolf Blog https://wolfblog.cn。我也非常感谢整个开源技术社区，我们站在巨人的肩膀上前行，共同创造新世界。
