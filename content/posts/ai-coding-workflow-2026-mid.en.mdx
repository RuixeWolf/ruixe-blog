...
```markdown
For the decision questions use your recommended options. (optional)

Keep digging deeper; do not create an OpenSpec change yet.

After exploration, judge by the task size whether an OpenSpec change is needed, or whether making the changes directly is recommended. (optional)
```

**OpenSpec Opsx workflow** - after exploration, for small tasks skip the proposal and change code directly.

```
Please exit Explore mode, do not create an OpenSpec change, and implement the changes directly, then update the related OpenSpec docs.
```
...
2. Refine the requirements based on the exploration results by answering the decision questions. It may take 1 ~ 3 rounds of conversation to refine the exploration until the plan meets expectations and no new decision questions or unclear points remain.
...
- Use MCP or Context7 etc. to fetch docs and best practices online
- Ask me when anything is unclear, to clarify requirements
- Split task.md into a detailed checklist for implementation across multiple AI sessions
```
...
2. Choose a solution based on the exploration results, or supply more information. It may take 1 ~ 3 rounds of conversation to refine the exploration until the plan meets expectations and no new decision questions or unclear points remain.
