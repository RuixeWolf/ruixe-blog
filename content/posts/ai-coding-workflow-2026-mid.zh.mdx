...
探索完成后按任务量评估是否需要创建 OpenSpec Change，是否推荐直接改动代码进行实现。（可选）
```

**OpenSpec Opsx 工作流** - 探索完成后对于小任务不创建 Proposal 直接改代码实现。

```
请退出 Explore 模式，不创建 OpenSpec 变更，直接改动代码进行实现，然后更新相关的 OpenSpec 文档。
```
...
2. 依据探索结果完善需求，回答决策问题，可能需要 1 ~ 3 次对话完善探索，直到方案符合预期，没有新的待决策问题或者不清晰的内容。
...
- 请使用 MCP 或 Context7 等工具联网获取文档与最佳实践
- 任务过程中遇到不明确的事物请向我提问，明确需求
- 请将 task.md 任务清单详细拆分，用于多个 AI Session 执行实现
```
...
2. 依据探索结果选择解决方案或者补充更多信息，可能需要 1 ~ 3 次对话完善探索，直到方案符合预期，没有新的待决策问题或者不清晰的内容。
