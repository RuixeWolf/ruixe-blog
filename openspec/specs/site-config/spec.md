...
定义 Ruixe Blog 站点标识配置（GitHub 用户名、站点标题、站点描述）的来源与加载方式。配置以 YAML 文件（`content/site.yaml`）为单一来源，随代码提交至 Git，避免环境变量在开发机与部署环境间漂移。`lib/site-config.ts` 作为 Server-only 模块提供统一访问入口 `siteConfig`，客户端组件通过 prop 接收配置值，不直接依赖配置模块。
