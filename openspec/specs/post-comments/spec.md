...
为 Ruixe Blog 的文章详情页提供基于 Giscus（GitHub Discussions）的评论能力。评论区作为 Client Component 渲染于文章正文之后，配置值来自 `content/site.yaml` 的 `giscus` 块并通过 props 注入（遵循 RSC->Client 边界）。评论主题、UI 语言与博客的 `next-themes` 主题、`next-intl` locale 实时同步，不同语言版本的文章拥有独立评论区。
