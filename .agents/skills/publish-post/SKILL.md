...
7. When publishing a new post from `drafts/*.md`, the draft's own language
   variant is SACRED: copy the body verbatim (only inserting frontmatter
   above it). NEVER rewrite, rephrase, polish, reformat, or "fix" the
   author's wording, punctuation, Markdown, or code - not even typos -
   unless the user explicitly asks for that specific change. The only
   exception is other-locale variants, which are generated by faithfully
   translating the untouched draft body (see Step 3a.5).
...
4. **Write the primary locale file**: `content/posts/{slug}.{locale}.mdx`. For
   ...
   body. The body MUST be copied character-for-character from the draft - do
   not rewrite, rephrase, reformat, reorder, or "improve" anything (typos
   included). The only permitted body change is removing a leading H1 that
   duplicates `title` (ask the user first; site chrome already renders the
   title). If you believe the draft needs edits, list them as SUGGESTIONS in
   your summary and let the user apply them - do not apply them yourself.
5. **Create other locale variants**: these are the ONLY files whose body you
   generate. Translate the primary body faithfully - preserve the author's
   structure, tone, emphasis, and code blocks; translate, never rewrite - and
   localize `title`/`description`. Keep `publishedTime`, `category`, `tags`
   identical across variants. Mark the translations as needing human review
